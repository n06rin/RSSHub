...
module.exports = async (ctx) => {
    ...
    const item = await Promise.all(
        list.toArray().map(async (item) => {
            ...
            const link = `${ABSOLUTE_URL}${item.find('h3 a').first().attr('href')}`;
            ...
            const [description, author, pubDate] = await ctx.cache.tryGet(link, async () => {
                ...
                const articleImageSrc = $articlePage('article img').first().attr('src');

                const articleFirstParagraph = $articlePage('p b').first().text();

                const subtitle = $articlePage('.i-date > span');
                const pubDate = getParsedPubDate(cheerio(subtitle.get(0)).text())
                    .utc()
                    .format();
                const author = cheerio(subtitle.get(1)).text();

                return [`<img src="${articleImageSrc}"<br />${articleFirstParagraph}`, author, pubDate];
            });
